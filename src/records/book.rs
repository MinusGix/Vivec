use super::{
    common::{lstring::LString, CommonRecordInfo, FromRecord, TypeNamed},
    fields::{
...
use crate::{
    ...
    parse::{PResult, Parse},
    util::{DataSize, Writable}, collect_one,
};
	...
	fn from_record(record: GeneralRecord<'data>) -> PResult<Self, FromRecordError<'data>> {
		...
		while let Some(field) = field_iter.next() {
			match field.type_name().as_ref() {
				...
				b"VMAD" => collect_one!(vmad::VMAD<'data, vmad::NoFragments>, field => fields; vmad_index),
